...
What follows is a list of steps a maintainer of a datatank has to take in order to create a new definition so that data can be published from a certain data file. This small tutorial assumes that a datatank is hosted on http://localhost and explains the basics of the discovery document. A fair bit of reading, but essential nonetheless!

If you're not a technical person or not looking for a way to use our API but rather want to publish data through our [user interface](/4.1/ui_introduction), please do so. The following paragraphs form a technical outline on how to build your own UI or business logic on top of our internal API.

### Set up the resource definition template

In order to know which meta-data properties you have to pass to create a valid resource definition in order to publish data from a certain data structure you'll have to take a look at the discovery document, located at http://localhost/discovery.
...
* http://localhost/api/definitions/trees/2011
* http://localhost/api/definitions/trees/2012
* http://localhost/api/definitions/trees/2013
...
$url = "http://localhost/api/definitions/trees/2011";
